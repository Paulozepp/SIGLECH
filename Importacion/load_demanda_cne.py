#!/usr/bin/env python3
"""
🚀 Cargador de Demanda CNE para SIGLECH
Carga archivo CSV pesado en MySQL con chunking y optimizaciones
"""
...
class CargadorDemandaCNE:
    ...
    def crear_tabla(self):
        ...
        self.log_info("Verificando/creando tabla demanda_cne...")

        sql = """
        CREATE TABLE IF NOT EXISTS demanda_cne (
        ...
        try:
            ...
            self.conn.commit()
            self.log_exito("Tabla demanda_cne lista")
            return True
        except Error as e:
    ...
    def cargar_por_chunks(self, archivo, chunk_size=2000):
        ...
        try:
            ...
            self.log_exito(f"¡Carga completada! {total_cargadas:,} registros en demanda_cne")
            return True
    ...
    def mostrar_resumen(self):
        ...
        # Total
        self.cursor.execute("SELECT COUNT(*) FROM demanda_cne")
...
def main():
    parser = argparse.ArgumentParser(description="Cargador de Demanda CNE")
    ...
    archivo_csv = r"C:\xampp\htdocs\SIGLECH\Importacion\Lista de espera Julio 2026\TIPO DE LE\CNE\CSV\Demanda_Total_CNE.csv"

    print(f"\n{ColoresTerminal.BOLD}{ColoresTerminal.MAGENTA}")
    print("╔════════════════════════════════════════╗")
    print("║  🚀 CARGADOR DE DEMANDA CNE - SIGLECH  ║")
    print("╚════════════════════════════════════════╝")
    print(f"{ColoresTerminal.RESET}\n")

    cargador = CargadorDemandaCNE(
        host=args.host,
        user=args.user,
