#!/usr/bin/env python3
"""
🚀 Cargador de Demanda IQ (Intervenciones Quirúrgicas) para SIGLECH
...
class CargadorDemandaIQ:
    ...
    def crear_tabla(self):
        ...
        self.log_info("Verificando/creando tabla demanda_iq...")

        sql = """
        CREATE TABLE IF NOT EXISTS demanda_iq (
        ...
        try:
            ...
            self.conn.commit()
            self.log_exito("Tabla demanda_iq lista")
            return True
        except Error as e:
    ...
    def cargar_por_chunks(self, archivo, chunk_size=2000):
        ...
        try:
            ...
            self.log_exito(f"¡Carga completada! {total_cargadas:,} registros en demanda_iq")
            return True
    ...
    def mostrar_resumen(self):
        ...
        # Total
        self.cursor.execute("SELECT COUNT(*) FROM demanda_iq")
...
def main():
    parser = argparse.ArgumentParser(description="Cargador de Demanda IQ")
    ...
    archivo_csv = r"C:\xampp\htdocs\SIGLECH\Importacion\Lista de espera Julio 2026\TIPO DE LE\IQ\CSV\Demanda_Total_IQ.csv"

    print(f"\n{ColoresTerminal.BOLD}{ColoresTerminal.MAGENTA}")
    print("╔════════════════════════════════════════╗")
    print("║  🚀 CARGADOR DE DEMANDA IQ - SIGLECH   ║")
    print("╚════════════════════════════════════════╝")
    print(f"{ColoresTerminal.RESET}\n")

    cargador = CargadorDemandaIQ(
        host=args.host,
        user=args.user,
